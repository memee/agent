...
from the profile's sandbox config, and validate `image_url` with `http_url_validator`
before starting the sub-agent, raising `PermissionError` if the URL is invalid or
resolves to a private IP.

It SHALL spawn the sub-agent via `delegate_async()`, then `await` its completion event,
and return the sub-agent's final text response as a string to the orchestrator.

The sub-agent SHALL be initialized with: the profile's system prompt, the task as the
first user message (plain text or multimodal if `image_url` is provided), the profile's
model, and the profile's tool list.
...
- **THEN** a child agent is spawned asynchronously using the researcher's profile, and its final text is returned as the tool result once complete
...
- **THEN** the child agent receives only those two tools, not the full registry
...
- **THEN** the child agent's first user message is a multimodal content list with the task text and the image URL
...
- **THEN** raises `PermissionError` before any child agent is spawned

#### Scenario: No image_url produces a plain text first message

- **WHEN** `delegate` is called without `image_url` (or `image_url=None`)
- **THEN** the child agent's first user message is a plain string matching `task`
