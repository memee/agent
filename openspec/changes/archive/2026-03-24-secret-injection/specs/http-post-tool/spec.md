...
It SHALL accept `url: str`, `body: str` (a JSON-encoded string), and `headers: str` (an optional JSON-encoded object string, defaulting to `"{}"`).
...
#### Scenario: POST with custom headers sends them in the request

- **WHEN** `http_post` is called with `headers='{"Authorization": "Bearer tok123"}'`
- **THEN** the outgoing request includes the `Authorization` header with value `"Bearer tok123"`

#### Scenario: POST with default empty headers works normally

- **WHEN** `http_post` is called without a `headers` argument
...
- **WHEN** `http_post` is called with a `headers` string that is not valid JSON
