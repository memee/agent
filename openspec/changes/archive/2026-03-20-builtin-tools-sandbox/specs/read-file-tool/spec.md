...
The library SHALL register a `read_file` tool in the `"builtin"` group. It SHALL accept a single `path: str` argument and return the file's text content as a string.
...
It SHALL raise `ValueError` if the resolved file exceeds `sandbox.max_file_bytes`.
