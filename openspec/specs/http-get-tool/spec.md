...
The library SHALL register an `http_get` tool in the `"builtin"` group with `domain="http"`. It SHALL accept a single `url: str` argument and return the response body as a string.
